...
class BF-CapsBlock(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, vec_length = 16):
        super(BF-CapsBlock, self).__init__()
        ...
        self.conv1 = MultiConvCaps(in_capsules=in_capsules, out_capsules=out_capsules,
                                          ...
                                          nonlinearity="sqaush", batch_norm = False,
...
class CapsGenerator(nn.Module):
    def __init__(self, inchannels=1,block=BF-CapsBlock, layers=[2, 2, 2, 2],
                 num_classes=1,
        ...
        self.conv1 = nn.Conv2d(inchannels, self.inplanes, kernel_size=5, stride=1, padding=2, bias = False)
  ...
  self.layer5 = self._make_layer(block, 512, layers[4], stride=2,vec_length=self.vec_length)
        self.filters = [32,32,64,128,256,512]
  self.deconvcaps5 = deconvolutionalCapsule(
...
if __name__ == "__main__":
    ...
    tmp = torch.randn(4, 1, 512, 1024).cuda()
